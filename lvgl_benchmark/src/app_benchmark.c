...
#if defined(ULMK_BOARD_PSRAM_BASE)
#include "psram_stress.h"
#endif

void app_benchmark_run(ulmk_dev_t *disp_dev, ulmk_dev_t *input_dev);

#ifndef ULMK_BANNER_VERSION
#define ULMK_BANNER_VERSION	"0.3.0"
#endif
#ifndef ULMK_BANNER_BSP
#define ULMK_BANNER_BSP		"ulmk"
#endif
...
void app_benchmark_run(ulmk_dev_t *disp_dev, ulmk_dev_t *input_dev)
{
	...
	if (!disp_dev || !input_dev) {
		board_console_puts("lvgl: missing devices\r\n");
		ulmk_thread_exit();
	}
#if LV_MEM_ADR != 0
	...
	(void)port_indev_init(input_dev, disp);
