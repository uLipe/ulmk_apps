...
/*
 * silicon_device_manager — echo / display / input dummies through
 * ulmk_device_manager (HIL).
 *
 * Expect: SILICON_DEVICE_MANAGER: PASS
 */
#include <stdint.h>
#include <stddef.h>
#include <string.h>
...
static ULMK_PRIVATE ulmk_ep_t g_echo_ep;
static ULMK_PRIVATE uint8_t g_echo_buf[ECHO_BUF];
static ULMK_PRIVATE uint32_t g_echo_len;
static ULMK_PRIVATE ulmk_tid_t g_echo_tid;

static ULMK_PRIVATE ulmk_ep_t g_disp_ep;
static ULMK_PRIVATE ulmk_tid_t g_disp_tid;
static ULMK_PRIVATE uint16_t g_disp_fb[2][DISP_FB_WORDS];
static ULMK_PRIVATE int g_disp_on;

static ULMK_PRIVATE ulmk_ep_t g_input_ep;
static ULMK_PRIVATE ulmk_tid_t g_input_tid;
...
void ulmk_root_thread(const ulmk_boot_info_t *info)
{
	...
	board_console_puts("silicon_device_manager: start\n");
	g_pass = 0;
	g_fail = 0;
	g_echo_ep = ULMK_EP_INVALID;
	g_echo_tid = ULMK_TID_INVALID;
	g_disp_ep = ULMK_EP_INVALID;
	g_disp_tid = ULMK_TID_INVALID;
	g_input_ep = ULMK_EP_INVALID;
	g_input_tid = ULMK_TID_INVALID;
	g_disp_on = 0;
	...
	if (map_ok(mapped)) {
		...
		CHECK("read_grant",
		      n == 48 && out[0] == 0u && out[47] == 47u);
		(void)ulmk_mem_unmap(mapped, 64u);
	}
	...
	board_console_puts("silicon_device_manager: pass=");
	put_u32((uint32_t)g_pass);
	board_console_puts(" fail=");
	put_u32((uint32_t)g_fail);
	board_console_puts("\n");
	board_console_puts(g_fail == 0 ? "SILICON_DEVICE_MANAGER: PASS\n" :
